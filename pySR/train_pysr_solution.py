...
def train(X, y):
    ...
    model = PySRRegressor(
        ...
        #loss="loss(prediction, target) = (1/(1+exp(-prediction))-target)^2"
...
def test(X, y, model, output_path):
    ...
    plot_true_vs_pred(y, y_pred,"SymbolicRegression",  output_path, R_square, RMSE, set_ylim=False)

if __name__ =="__main__":
    pysr.julia_helpers.init_julia()
    parser = argparse.ArgumentParser(description='')
    parser.add_argument('--output_path', type=str, default='../result/42/01-22/solute_polarity_index/final')
    ...
    save_folder = os.path.join(args.output_path, 'pysr')
